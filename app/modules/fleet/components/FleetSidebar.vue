...
<template>
  <aside class="w-64 bg-white border-r border-slate-200 hidden md:flex flex-col h-full sticky top-0">
    <div class="p-6 border-b border-slate-100">
      <h1 class="text-xl font-bold text-slate-900 flex items-center gap-2">
        <TruckIcon class="w-6 h-6 text-primary-600" />
        Gestão de Frota
      </h1>
      <p class="text-xs text-slate-500 mt-1">Secretaria de Saúde</p>
    </div>
    ...
    <nav class="flex-1 p-4 space-y-1">
      <a 
        ...
        :class="item.active ? 'bg-primary-50 text-primary-700' : 'text-slate-600 hover:bg-slate-50 hover:text-slate-900'"
      >
    ...
    <div class="p-4 border-t border-slate-100" v-if="stats">
      <div class="bg-slate-50 rounded-lg p-4 space-y-3">
        <div class="text-xs font-semibold text-slate-500 uppercase mb-2">Resumo da Frota</div>
        
        <div class="flex justify-between items-center text-sm">
            <span class="text-slate-600">Total</span>
            <span class="font-bold text-slate-900">{{ stats.total }} Veículos</span>
        </div>
        
        <div class="w-full h-1 bg-slate-200 rounded-full overflow-hidden flex">
        ...
        <div class="grid grid-cols-3 gap-1 text-[10px] text-center pt-1">
            <div class="bg-green-50 text-green-700 rounded py-1 px-1">
                <div class="font-bold">{{ stats.available }}</div>
                <div>Disp</div>
            </div>
            <div class="bg-amber-50 text-amber-700 rounded py-1 px-1">
                <div class="font-bold">{{ stats.maintenance }}</div>
                <div>Manut</div>
            </div>
            <div class="bg-red-50 text-red-700 rounded py-1 px-1">
