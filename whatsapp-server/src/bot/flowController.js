...
export async function handleMessage(message, sock) {
  ...
  try {
    ...
    if (result.dataToSave) {
       ...
       await updateRegistration(conversation_id, currentData)
    }
    ...
    if (nextStep) {
      ...
      if (messageToSend) {
        ...
        if (nextConfig.type === 'info' && nextConfig.action === 'complete_registration') {
             await updateRegistration(conversation_id, { status: 'pending' })
      ...
      const saveResult = await saveFinalRegistration(conversation_id, currentData)
...
async function saveFinalRegistration(conversationId, flowData) {
...
async function updateRegistration(conversationId, data) {
