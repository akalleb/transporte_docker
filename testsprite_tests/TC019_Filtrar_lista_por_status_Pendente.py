...
async def run_test():
    ...
    try:
        ...
        # -> Type the provided email into the E-mail field, then type the password into the Senha field, and click 'Entrar na Plataforma'.
        ...
        frame = context.pages[-1]
        await expect(frame.locator('text=Pendente').first).to_be_visible(timeout=3000)
        await expect(frame.locator('xpath=//h1[text()="Lista de agendamentos"]').first).to_be_visible(timeout=3000)
