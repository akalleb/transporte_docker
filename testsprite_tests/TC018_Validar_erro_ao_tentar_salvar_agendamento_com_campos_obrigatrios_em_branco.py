...
async def run_test():
    ...
    try:
        ...
        # -> Fill the email (index 8) and password (index 9) fields, then click the Entrar button (index 14) to log in.
        ...
        # -> Fill the email and password fields again (if needed) and click the 'Entrar na Plataforma' button to attempt login, then wait for the dashboard to load.
        frame = context.pages[-1]
        # Input text
        elem = frame.locator('xpath=/html/body/div[1]/div/div[2]/div[2]/form/div[1]/div/input').nth(0)
        await page.wait_for_timeout(3000); await elem.fill('arthurkalleb@protonmail.com')
        ...
        await expect(frame.locator('text=Solicitante').first).to_be_visible(timeout=3000)
